  ...
  return (
    <Container id="repositories">
      ...
      <div className="flex flex-col gap-6">
        ...
        {isLoading && (
          <div className="flex flex-col gap-4">
            {Array.from({ length: 4 }).map((_, idx) => (
              <Skeleton
                key={`skeleton-${idx}`}
                className="h-28 w-full rounded-lg bg-muted"
              />
            ))}
          </div>
        )}
        ...
        {/* Error State */}
        {error && !isLoading && (
          <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-destructive/30 bg-destructive/10 p-6 text-center">
            <p className="text-sm font-para text-destructive">{error}</p>
        ...
        {!hasMore && !isLoading && repos.length > 0 && (
          <p className="pt-2 text-center text-xs text-muted-foreground font-mono">
