...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text items-center justify-center gap-1.5 py-2 text-xs font-para font-medium text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 **:data-[slot=kbd]:rounded-sm **:data-[slot=kbd]:px-1.5 **:data-[slot=kbd]:text-xs [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
        "inline-start":
...
const inputGroupButtonVariants = cva(
  "flex items-center gap-2 rounded-md text-xs font-para shadow-none",
  {
    variants: {
      size: {
...
function InputGroupText({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "flex items-center gap-2 font-para text-xs text-muted-foreground [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
