...
const advancePointer = (pointer: PointerState, elapsed: number): void => {
    ...
    pointer.presenceVelocity =
        (pointer.presenceVelocity -
            presenceResponse * presenceMomentum * elapsed) *
        presenceDecay;
...
const SHAPES = array<vec3f, ${ASCII_GLYPHS.length * 2}>(
  ${ASCII_SHAPES.flatMap((shape) => [shape.slice(0, 3), shape.slice(3)])
      .map(
          (part) =>
              `vec3f(${part.map((value) => value.toFixed(6)).join(", ")})`,
      )
      .join(",\n  ")}
);
...
const createRenderGraph = (
    ...
    const blurParamsX = uniforms(gpu, {
        direction: [1 / bloomSize[0], 0, 0, 0],
    });
    const blurParamsY = uniforms(gpu, {
        direction: [0, 1 / bloomSize[1], 0, 0],
    });
    ...
    const bloomBlurY = effect(gpu, BLOOM_BLUR_SHADER, {
        ...
        set: {
            bloomTexture: bloomScratchTarget,
            linearSampler,
            blur: blurParamsY,
        },
    });
...
export default function AeroShards({
    ...
    const resolvedMaterial =
        MATERIAL_PRESETS[material] || MATERIAL_PRESETS.pearl;
    ...
    const currentSettings = {
        ...
        scale: resolvedScale,
        stretch:
            resolvedStretch * (1 + Math.min(resolvedSpeed * 0.34, 1.2) * 0.1),
        speed: resolvedSpeed,
        spin: resolvedSpin,
    ...
    useEffect(() => {
        ...
        const updatePointerTarget = (next: [number, number]) => {
            ...
            if (
                !pointer.initialized ||
                (!pointer.active && pointer.presence === 0)
            ) {
            ...
            try {
                ...
                const resolveFrameState = (now: number): FrameState => {
                    ...
                    if (
                        ...
                        Math.abs(settingsRef.current!.scale - renderScale) >
                            0.001
                    ) {
                ...
                const resizePostTargets = (
                    qualityLevel = appliedBloomLevel,
                ) => {
                    ...
                    graph.blurParamsX.set({
                        direction: [1 / bloomSize[0], 0, 0, 0],
                    });
                    graph.blurParamsY.set({
                        direction: [0, 1 / bloomSize[1], 0, 0],
                    });
                ...
                const resizeOutput = () => {
                    ...
                    if (
                        nextSize[0] !== output.size[0] ||
                        nextSize[1] !== output.size[1]
                    )
                ...
                const renderFrame = (currentFrame: Frame) => {
                    ...
                    layoutTransitioning =
                        layoutWeights[settings.composition] !== 1;
                    if (!frozen) {
                        const travelAspect =
                    ...
                    const activeCount = Math.max(
                        ...
                        Math.round(
                            preset.count *
                                settings.detailCount *
                                runtimeQuality.countScale,
                        ),
                    );
                    ...
                    const pointerWorldY =
                        (1 - pointer.position[1] * 2) / renderScale;
                    ...
                    if (!postEnabled) {
                        ...
                        if (settings.bloom > 0.0001) {
                            ...
                            currentFrame.pass(
                                {
                                    target: graph.bloomScratchTarget,
                                    clear: [0, 0, 0, 1],
                                },
                                (pass) => {
                ...
                const scheduleSleep = (targetTimestamp: number) => {
                    ...
                    const delay = Math.max(
                        0,
                        targetTimestamp - performance.now() - 10,
                    );
                ...
                const scheduleFrame = (timestamp: number) => {
                    ...
                    const settingsChanged =
                        settings.signature !== lastSettingsSignature;
                    const frozen =
                        settings.paused ||
                        reduceMotion.matches ||
                        settings.speed <= 0.0001;
                    ...
                    const sustainedPressure =
                        pressureStartedAt > 0 &&
                        monitorNow - pressureStartedAt > 1800;
                    const qualityCooldownComplete =
                        monitorNow - lastQualityChange > 2200;
                    if (
                    ...
                    nextPresentationTimestamp = advanceFrameDeadline(
                        ...
                        forceFrame ||
                            nextState.continuous !== frameState.continuous,
                    );
                    if (frozen) return;
